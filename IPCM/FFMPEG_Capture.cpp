...
bool CCapture_FFMPEG::open(const char* _filename)
{
	...
	av_dump_format(ic, 0, _filename, 0);
...
double CCapture_FFMPEG::get(int property_id) const
{
	...
	switch (property_id)
	{
	case FFMPEG_CAP_PROP_CODEC_ID:
		return (double)ic->streams[video_stream]->codec->codec_id;
	case FFMPEG_CAP_PROP_POS_AVI_RATIO:
		return r2d(ic->streams[video_stream]->time_base);
	case FFMPEG_CAP_PROP_FRAME_WIDTH:
		return (double)ic->streams[video_stream]->codec->width;
	case FFMPEG_CAP_PROP_FRAME_HEIGHT:
		return (double)ic->streams[video_stream]->codec->height;
