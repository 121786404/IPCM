...
void CIPCMDlg::OnBnClickedOpenUrl()
{
	...
	if (m_bUseFFmpegCapture)
	{
		...
		if (!m_capture_ffmpeg.isOpened())
		{
			MessageBox(_T("打开视频流错误"), _T("错误"), MB_ICONEXCLAMATION);
			return;
		}
	}
	else
...
void CIPCMDlg::OnBnClickedShowFps()
{
	...
	{
		m_bShowFpsSpline = false;
		GetDlgItem(IDC_SHOW_FPS)->SetWindowText(_T("显示FPS曲线"));
		cvDestroyWindow("fps");
	}
	else
	{
		m_bShowFpsSpline = true;
		GetDlgItem(IDC_SHOW_FPS)->SetWindowText(_T("关闭FPS曲线"));
...
void CIPCMDlg::OnBnClickedPreview()
{
	...
	{
		m_bPreview = true;
		GetDlgItem(IDC_PREVIEW)->SetWindowText(_T("停止预览"));
